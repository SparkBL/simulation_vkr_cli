...
{
private:
    static std::unordered_map<std::string, DelayFactory *> factories_;
...
class RQTNodeFactory : public ProducerFactory
{
    RQTNode *Create(nlohmann::json parameters) override
    {
        ...
        Delay *d2 = DelayTypeFactory::Create(parameters.at("mu_2").value("delay_type", "exponential"), parameters.at("mu_1"));
...
{
    Orbit *Create(nlohmann::json parameters) override
    {
