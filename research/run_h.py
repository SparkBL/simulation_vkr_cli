...
parser.add_argument('inp_p', type=float,
                    help='Input p')
parser.add_argument('inp_l1', type=float,
                    help='Input l1'),
parser.add_argument('inp_l2', type=float,
                    help='Input l2')
...
mod = {'input_intensity':[args.inp_p,args.inp_l1,args.inp_l2],'orbit_intesity':args.orb_i,'node_intensity':args.node_i}

model = rq.Model()
model.set_time(0) 
model.set_end(20000)
#Добавление элементов
model.add_producer(rq.SimpleInput(rq.HyperExponential([args.inp_p,1-args.inp_p],[args.inp_l1,args.inp_l2]),1,0),"input")
...
with open(f'results/mod_{fname}.json','w', encoding='utf-8') as ff:
